...
작업을 시작하기 전, 대상 문서(ROADMAP.md)의 현재 상태를 먼저 확인합니다.

- ROADMAP.md는 어디까지나 **개발 계획 문서**입니다. 만약 문서 전체(또는 대부분의 Phase/Task)가 이미 완료 보고서처럼 ✅ 완료 상태로만 작성되어 있고 앞으로 진행할 계획으로 기능하지 않는다면, 이를 계획 문서 형태로 되돌립니다.
- 이때 Task/Phase의 제목, 설명, 구현 사항 등 **내용은 그대로 유지**하고, 완료 상태 표시(✅, `- 완료`, `See: /tasks/...` 참조 등)만 계획 상태(상태 없음 또는 `- 우선순위`)로 되돌립니다.
- 실제로 완료가 확인된 작업(코드베이스 확인 등으로 검증 가능한 경우)까지 임의로 되돌리지는 않습니다. 완료 여부가 불확실한 경우에만 계획 상태로 되돌립니다.
